...
export const baseMetadata: Metadata = {
  ...
  title: {
    default: "Clinic GEO | 병·의원 GEO 아티클 허브",
...
export function buildMetadata(pathname: string, title?: string, description?: string): Metadata {
  ...
  const resolvedTitle = title ?? "Clinic GEO | 병·의원 GEO 아티클 허브";
  const resolvedDescription = description ?? baseMetadata.description ?? "Clinic GEO는 SUMMITFEED가 운영하는 병·의원 전용 AI 검색 최적화 아티클 허브입니다.";
